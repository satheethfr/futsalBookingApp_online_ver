...
export function AppProvider({ children }) {
      ...
      .on('postgres_changes', 
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            dispatch({ type: 'REALTIME_BOOKING_DELETED', payload: payload.old.id });
          }
        }
      )
      .subscribe((status) => {
      ...
      .on('postgres_changes',
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            dispatch({ type: 'REALTIME_CUSTOMER_DELETED', payload: payload.old.id });
          }
        }
      )
      .subscribe();
  ...
  const loadData = async () => {
    try {
      ...
      if (customersError || bookingsError) throw customersError || bookingsError;
      ...
      await AsyncStorage.setItem('cache_customers', JSON.stringify(transformedCustomers));
      await AsyncStorage.setItem('cache_bookings', JSON.stringify(transformedBookings));
      
      dispatch({ type: 'LOAD_DATA', payload: { customers: transformedCustomers, bookings: transformedBookings } });
    } catch (error) {
      ...
      try {
        ...
        if (cachedCustomers && cachedBookings) {
          dispatch({ type: 'LOAD_CACHED_DATA', payload: { 
